...
Throughout this workshop, and the `ebirdst` documentation, we frequently use eBird species codes (e.g. "buwtea" above). These alphanumeric codes uniquely identifying species in eBird and are typically of 6 letters, sometimes followed by a single number. You can always look these codes up in the `ebirdst_runs` data frame.
...
# plot the trajectory
ggplot(traj_buwtea, aes(x = week, y = prop_pop, color = species)) +
  geom_line() +
  scale_y_continuous(labels = scales::percent) +
  labs(x = "Week", 
       y = "% of population",
